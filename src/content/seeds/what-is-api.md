...
날씨 앱을 열면 기상청 서버에서 데이터가 온다. 카카오톡을 보내면 카카오 서버가 전달한다. 이 연결을 만드는 게 API다.

API는 Application Programming Interface의 약자다. 두 시스템이 대화하는 규칙이라고 보면 된다.
...
- **나(클라이언트)**: 요청을 보내는 쪽
- **카운터(API)**: 요청을 받고 결과를 돌려주는 중간 지점
- **바리스타(서버)**: 실제로 일을 처리하는 쪽

에스프레소 머신을 직접 조작할 필요 없다. 카운터에 말하면 된다. API도 마찬가지다.
...
날씨 하나 물어보는데 이 정도다. XML 봉투 안에 봉투를 넣고, 그 안에 요청을 넣는다. 은행에서는 보안 표준화 때문에 아직 쓰지만, 웹 개발자 입장에서는 배보다 배꼽이 컸다.
...
Facebook이 2015년에 공개했다. REST의 문제는 데이터를 너무 많이 받거나 적게 받는 것이다. 유저 이름만 필요한데 `GET /users/42`를 호출하면 주소, 전화번호까지 딸려온다.
...
필요한 필드만 콕 집어서 요청하면 딱 그만큼만 온다. 모바일처럼 대역폭이 아쉬운 환경에서 차이가 크다.
...
Postman은 API를 테스트하는 GUI 도구다. Chrome 확장으로 시작해서 지금은 독립 앱이 됐다. 코드 없이 API를 쏴볼 수 있어서 개발자부터 PM까지 쓴다. 터미널에서 curl로도 된다.

```bash
curl https://jsonplaceholder.typicode.com/posts/1
```

이걸 치면 JSON이 바로 나온다. 그런데 헤더와 바디를 붙이기 시작하면 커맨드라인이 길어진다. Postman은 이걸 GUI로 해준다.
...
HTTP 메서드는 GET(조회), URL은 `/posts/1`, 응답 코드는 200 OK. 응답 바디가 JSON 객체다.
...
Postman에서 Send를 누르면 응답이 온다. 그 사이의 경로는 이렇다.
...
API가 느릴 때 "API가 느려요"로는 디버깅이 안 된다. "로드밸런서에서 3번 서버로 가는 요청이 느려요"라고 말해야 고칠 수 있다.
