...
AI 에이전트의 컨텍스트 윈도우는 한정된 자원이다. 200K 토큰 안에서 각 레이어가 순서대로 자리를 차지한다.
...
정적인 것이 먼저 들어가고, 동적인 것이 나중에 들어간다. 이 순서가 맞아야 프롬프트 캐싱이 작동한다.

## 인터랙티브 데모

슬라이더로 컨텍스트 윈도우를 줄여보면 어떤 레이어부터 잘리는지 보인다.
